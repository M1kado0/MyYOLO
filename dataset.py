...
class YoloDataset(data.Dataset):
    def __init__(self, foldername=Path(), imgsz=640, names={}, augment=False):
        ...
        # Read labels
        filenames = sorted(str(p) for p in Path(foldername).rglob("*") if p.is_file())
...
def xywhn2xyxy_torch(x, w, h, pad_w=0, pad_h=0):
    """
    Converts nx4 boxes from [x, y, w, h] normalized to [x1, y1, x2, y2] pixel coordinates.
    Uses PyTorch operations for GPU compatibility.
    """
    y = x.clone()
    y[:, 0] = w * (x[:, 0] - x[:, 2] / 2) + pad_w  # top left x
    y[:, 1] = h * (x[:, 1] - x[:, 3] / 2) + pad_h  # top left y
